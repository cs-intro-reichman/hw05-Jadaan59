...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		System.out.println(count(board, 2, 4));
		System.out.println(cellValue(board, 2, 4)); 
	}
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		int ValueOfCell = 0;
		if (board [i][j] == 1){
		if (count(board,i,j) < 2) { ValueOfCell = 0; }
		if (count(board , i , j) == 2 || count(board, i, j) == 3) { ValueOfCell = 1;}
		if (count(board, i, j) > 3) { ValueOfCell = 0;}
	}
    ...
    public static void print(int[][] arr) {
		for (int i = 1; i < arr.length - 1; i++){
			for (int j = 1; j < arr[0].length - 1; j++){
                System.out.printf("%3s",(int) arr[i][j]);
        }
				System.out.println();
	}
}
	...
	public static void show(int[][] board) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				...
				StdDraw.setPenColor(color, color, 50);
